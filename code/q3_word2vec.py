...
def normalizeRows(x):
    ...
    ### YOUR CODE HERE
    N = x.shape[0]
    x = x/np.linalg.norm(np.array(x), axis=1).reshape(N,1)
    ### END YOUR CODE
...
def negSamplingCostAndGradient(predicted, target, outputVectors, dataset,K=10):
    ...
    #print vecs.shape # 11*3
    #print predicted.shape # 3*1
    tem = np.dot(vecs,predicted) * labels
    #print tem.shape
    t = sigmoid(tem)
    #print t.shape
    #print labels.shape # 11*1
...
def skipgram(currentWord, C, contextWords, tokens, inputVectors, outputVectors, 
    ...
    contextCost, contextGradPred, contextGrad = word2vecCostAndGradient(predicted, target, outputVectors,dataset)
    cost += contextCost
